export type BPLevel = "crisis" | "high2" | "high1" | "elevated" | "normal";
...
export function getBPLevel(systolic: number, diastolic: number) {
  if (
    systolic >= BP_THRESHOLDS.sys.crisis ||
    diastolic >= BP_THRESHOLDS.dia.crisis
  )
    return "crisis";
  if (
    systolic >= BP_THRESHOLDS.sys.high2 ||
    diastolic >= BP_THRESHOLDS.dia.high2
  )
    return "high2";
  if (
    systolic >= BP_THRESHOLDS.sys.high1 ||
    diastolic >= BP_THRESHOLDS.dia.high1
  )
    return "high1";
  if (
    systolic >= BP_THRESHOLDS.sys.elevated ||
    diastolic >= BP_THRESHOLDS.dia.elevated
  )
    return "elevated";
  return "normal";
}
...
export const grayButtonStyle =
  "bg-gray-300 dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-1 border-[1px] border-gray-300 dark:border-gray-700 rounded shadow-md hover:bg-gray-400 dark:hover:bg-gray-600 disabled:opacity-60 disabled:animate-none transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]";
